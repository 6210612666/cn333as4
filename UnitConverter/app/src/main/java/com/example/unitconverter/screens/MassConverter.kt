...
fun MassConverter() {
    ...
    val strton = stringResource(id = R.string.ton)
    val currentValue = viewModel.mass.observeAsState(viewModel.mass.value ?: "")
    val weight = viewModel.weight.observeAsState(viewModel.weight.value ?: R.string.kilogram
    )
    ...
    Column(
        ...
        MassScaleButtonGroup(
            selected = weight,
            modifier = Modifier.padding(bottom = 16.dp)
        ) { resId: Int -> viewModel.setWeight(resId) }
        Button(
            onClick = calc,
            enabled = enabled
        ) {
